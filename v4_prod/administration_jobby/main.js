...
app.get('/categories', function(request, response) {
	...
	if (request.session.loggedin) {
		connection.query('SELECT * FROM accounts WHERE username = ?', [request.session.username], function(error, results, fields) {
			...
			response.render('categories.html', { account: results[0] });
		});
	} else {
		// Redirect to login page
		response.redirect('/');
...
app.get('/category', function(request, response) {
	...
	if (request.session.loggedin) {
		...
		connection.query('SELECT * FROM accounts WHERE username = ?', [request.session.username], function(error, results, fields) {
			...
			response.render('category.html', { account: results[0] });
		});
	} else {
		// Redirect to login page
		response.redirect('/');
...
app.get('/question', function(request, response) {
	...
	if (request.session.loggedin) {
		...
		connection.query('SELECT * FROM accounts WHERE username = ?', [request.session.username], function(error, results, fields) {
			...
			response.render('question.html', { account: results[0] });
		});
	} else {
		// Redirect to login page
		response.redirect('/');
...
app.get('/reponse', function(request, response) {
	// Check if user is logged in
	if (request.session.loggedin) {
		
		connection.query('SELECT * FROM accounts WHERE username = ?', [request.session.username], function(error, results, fields) {
			
			response.render('reponse.html', { account: results[0] });
		});
	} else {
		// Redirect to login page
		response.redirect('/');
