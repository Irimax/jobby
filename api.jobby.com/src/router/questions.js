...
router.get('/',
  ...
  async (req, res, next) => {
    let sql = `SELECT q.questionLabel AS title, q.questionId AS id,q.questionOrder AS myorder, qt.questionTypeLabel AS type
    FROM  question AS q 
    INNER JOIN questionType AS qt ON q.question_questionTypeId = qt.questionTypeId  
