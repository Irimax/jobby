...
async function createQuestion () {
  ...
  let responsePFEnv
  try {
    responsePFEnv = await fetch('http://127.0.0.1:11010/createQuestion', {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        ...
        question_categoryId: searchParams.get('categoryId')
  ...
  if (responsePF.messageError === false) {
    ...
    let blockMessage = document.querySelector('#message')
    let questionId = responsePF.datas
    document.location.href = 'http://127.0.0.1:5001/Jobby/jobby_admin_v1/views/question.html?questionId=' + questionId
    blockMessage.className = ''
    blockMessage.innerHTML = message
    blockMessage.className += ' alert'
